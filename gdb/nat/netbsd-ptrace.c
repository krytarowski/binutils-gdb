...
/* Test broken off-trunk netbsd kernel patchset for NX support on i386.  It was
   removed in Fedora kernel 88fa1f0332d188795ed73d7ac2b1564e11a0b4cd.

   Test also x86_64 arch for PaX support.  */

static void
netbsd_ptrace_test_ret_to_nx (void)
{
#if defined __i386__ || defined __x86_64__
  pid_t child, got_pid;
  ...
  if (return_address == MAP_FAILED)
    {
      warning (_("netbsd_ptrace_test_ret_to_nx: Cannot mmap: %s"),
	       safe_strerror (errno));
      return;
    }
  ...
  *return_address = 0xcc;
  ...
  switch (child)
    {
    case -1:
      warning (_("netbsd_ptrace_test_ret_to_nx: Cannot fork: %s"),
	       safe_strerror (errno));
      return;
    ...
    case 0:
      ...
      if (l != 0)
	warning (_("netbsd_ptrace_test_ret_to_nx: Cannot PTRACE_TRACEME: %s"),
		 safe_strerror (errno));
      else
	{
#if defined __i386__
	  asm volatile ("pushl %0;"
			".globl netbsd_ptrace_test_ret_to_nx_instr;"
			"netbsd_ptrace_test_ret_to_nx_instr:"
			"ret"
...
# error "!__i386__ && !__x86_64__"
  ...
  if (got_pid != child)
    {
      warning (_("netbsd_ptrace_test_ret_to_nx: waitpid returned %ld: %s"),
  ...
  if (WIFSIGNALED (status))
	...
	warning (_("netbsd_ptrace_test_ret_to_nx: WTERMSIG %d is not SIGKILL!"),
		 (int) WTERMSIG (status));
      else
	warning (_("Cannot call inferior functions, netbsd kernel PaX "
  ...
  if (!WIFSTOPPED (status))
    {
      warning (_("netbsd_ptrace_test_ret_to_nx: status %d is not WIFSTOPPED!"),
	       status);
      kill_child (child, "netbsd_ptrace_test_ret_to_nx");
      return;
    }
  ...
  if (WSTOPSIG (status) != SIGTRAP && WSTOPSIG (status) != SIGSEGV)
    {
      warning (_("netbsd_ptrace_test_ret_to_nx: "
  ...
  if (ptrace (PTRACE_GETREGS, child, (PTRACE_TYPE_ARG3) 0,
    ...
    {
      warning (_("netbsd_ptrace_test_ret_to_nx: Cannot PTRACE_GETREGS: %s"),
	       safe_strerror (errno));
    }
#if defined __i386__
...
# error "!__i386__ && !__x86_64__"
  ...
  if ((void (*) (void)) pc != &netbsd_ptrace_test_ret_to_nx_instr)
    warning (_("netbsd_ptrace_test_ret_to_nx: PC %p is neither near return "
	       "address %p nor is the return instruction %p!"),
	     pc, return_address, &netbsd_ptrace_test_ret_to_nx_instr);
  else
    warning (_("Cannot call inferior functions on this system - "
	       "netbsd kernel with broken i386 NX (non-executable pages) "
	       "support detected!"));
#endif /* defined __i386__ || defined __x86_64__ */
}
